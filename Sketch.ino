...
void setup() {
  ...
  // NOTE: on classic ATmega328-based boards, attaching a Servo commandeers
  // Timer1, which also drives PWM on pins 9 and 10 (speedPinL/speedPinR).
  // This can affect motor PWM smoothness while the servo is attached.
  // Not changed here since pin reassignment was out of scope - just flagging
  // this for testing on your specific board.
  aimingServo.attach(servoPin);
  aimingServo.write(90); // Center servo position
  
  stopMotors();
  // NOTE: verify this polarity on your specific relay module before relying
  // on it unattended - some relay boards are active-low, some active-high.
...
void executeFiringSequence(int angle) {
  ...
  digitalWrite(motorIN3, LOW);
  digitalWrite(motorIN4, HIGH);
  delay(1100); // Adjust this delay time so it performs a clean 180-degree turn on your specific chassis
  stopMotors();
  delay(300);
