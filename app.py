...
# FIX: added threaded=True so the video stream doesn't block other requests
# on Flask's built-in dev server.
threading.Thread(
...
# Aiming servo range (matches Sketch.ino: center is 90, self-test sweeps 45-135)
SERVO_CENTER_ANGLE = 90
SERVO_MAX_SWING = 45  # degrees left/right of center
...
def loop():
    ...
    ret, frame = cap.read()
    if not ret:
        # FIX: log camera read failures instead of failing silently every cycle.
        print("[WARNING] Camera frame grab failed, skipping this cycle.")
        return
    ...
    if frame_counter % 2 == 0:
        ...
        # FIX: wrap inference so a single bad frame/tensor can't silently kill the loop.
        try:
        ...
        for row in predictions:
            ...
            if confidence > 0.65:
                ...
                if (current_time - last_fire_time) > COOLDOWN_SECONDS:
                    ...
                    # FIX: compute an actual aim angle from where the pest is in the
                    # frame (left/right of center), instead of a hardcoded value,
                    # and actually send it to the Arduino. This was previously
                    # commented out, so the rover never fired on a detection.
                    box_center_x = (x1 + x2) / 2
                    frame_center_x = w_orig / 2
                    offset_ratio = (box_center_x - frame_center_x) / frame_center_x  # -1..1
                    offset_ratio = max(-1.0, min(1.0, offset_ratio))
                    aim_angle = int(SERVO_CENTER_ANGLE + offset_ratio * SERVO_MAX_SWING)

                    try:
                        Bridge.call("engage", aim_angle)
                        print(f"[BRIDGE] Sent engage command with angle {aim_angle}")
                    except Exception as e:
                        print(f"[ERROR] Bridge call to 'engage' failed: {e}")
